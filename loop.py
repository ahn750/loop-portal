...
while True:
	...
	'''
	obtaining contour for outer loop and filling outer loop with black leaving only 
	the outermost background white
	'''
	contours=cv2.findContours(mask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[0]
	cv2.drawContours(outer, contours, -1,0, -1)
	

	#inverting the bckgnd to get the outer loop filled with white
